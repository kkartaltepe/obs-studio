...
static bool egl_make_current(EGLDisplay display, EGLSurface surface, EGLContext context)
{
	...
	if (!eglMakeCurrent(display, surface, surface, context)) {
		blog(LOG_ERROR, "eglMakeCurrent failed %s", gl_egl_error_to_string(eglGetError()));
		return false;
	}
...
static bool egl_context_create(struct gl_platform *plat, const EGLint *attribs)
{
	...
	plat->present_context = eglCreateContext(plat->display, plat->config, EGL_NO_CONTEXT, attribs);
	if (plat->present_context == EGL_NO_CONTEXT) {
		blog(LOG_ERROR, "eglCreateContext failed for present context");
		goto error;
	}
	plat->context = eglCreateContext(plat->display, plat->config, plat->present_context, attribs);
...
static void gl_wayland_egl_device_present_async(gs_device_t *device, struct gl_windowinfo *wi)
{
	struct gl_platform *plat = device->plat;
